...
public class WCUpdater
{
    ...
    public enum Condition { DOWNLOAD_PENDING, DOWNLOAD_COMPLETE, UPDATE_PENDING,
    	UP_TO_DATE, UPDATE_IS_AVAILABLE, UNAVAILABLE};
    ...
    public void setup(File webInfDir)
    {
        ...
        for(File jar : downloadDir.listFiles())
        	updateFileConditions.put(jar.getName(), Condition.DOWNLOAD_PENDING);

        for(File jar : stagingDir.listFiles())
        	updateFileConditions.put(jar.getName(), Condition.DOWNLOAD_COMPLETE);
	...
	/**
     * Creates an update by downloading the newest versions of the subsystems and preparing
     * the update directory with an update that can be applied.
     */
    ...
    private boolean downloadNewUpdates()
    {
        ...
        for (SubsystemUpdater thisUpdater : subsystems.values())
        {
            ...
            {
            	logError(getClass(), "Unable to read from provider.", e);
            }
            ...
            if (requires != null)
            {
                for (String requiredSubsystem : requires.split( ",\\s*" ))
                {
                    if (!subsystemsByName.containsKey(requiredSubsystem))
                    {
						...
						try {
							newSubsystem = thisUpdater.provider().subsystemDescriptor(
                                requiredSubsystem);
						} catch (IOException e) {
							newSubsystem = null;
						}
                        ...
                        else
                        {
                            try
                            {
                        		...
                        		{
                        			updateFileConditions.put(filename, Condition.DOWNLOAD_PENDING);
                        			newSubsystem.downloadTo(downloadDir, stagingDir);
                        			updateFileConditions.put(filename, Condition.DOWNLOAD_COMPLETE);

                        		}
                        		else
                        		{
                        			logInfo(newSubsystem.name + " is already downloaded.");
                        		}
                            }
                            catch(IOException c)
                            {
    ...
    private boolean downloadUpdateIfNecessary(SubsystemUpdater updater)
    {
        try
        {
        	...
        	if(latest != null)
        	{
        		String filename = latest.name + "_" + latest.currentVersion() + ".jar";
        		Condition fileCondition = updateFileConditions.get(filename);
        		...
        		{
        			updateFileConditions.put(filename, Condition.DOWNLOAD_PENDING);
        			if(updater.downloadUpdateIfNecessary(downloadDir, stagingDir))
        				updateFileConditions.put(filename, Condition.DOWNLOAD_COMPLETE);
        			else
        				updateFileConditions.put(filename, Condition.UP_TO_DATE);
        		}
        		else
        		{
    ...
    /**
     * Prepares for the update by moving all files from the staging directory to the update
     * directory
     */
    		...
    		for (File jar : stagingDir.listFiles())
            {
                    ...
                    SubsystemUpdater.JAVA_ARCHIVE_EXTENSIONS)
                {
                    if (jar.getName().endsWith(extension))
                    {
                		...
                		{
                    		logError( getClass(), "Unable to move " + jar.getName()
                    				+ " from " + stagingDir.getAbsolutePath() + " to "
                    				+ updateDir.getAbsolutePath());
                		}
                		else
                		{
                			updateFileConditions.put(jar.getName(), Condition.UPDATE_PENDING);
                			logInfo("Moving "+ jar.getName()
                    				+ " from " + stagingDir.getAbsolutePath() + " to "
                    				+ updateDir.getAbsolutePath());
                		}
                    }
    ...
    /**
     * Prepares for the update by moving all files from the staging directory to the update
     * directory. It only moves files that do not have requirements that have not been downloaded.
     */
    			...
    			if(jar != null)
    			{
    					...
    					SubsystemUpdater.JAVA_ARCHIVE_EXTENSIONS)
    				{
    					if (jar.getName().endsWith(extension))
    					{
    						...
    						subsystemName = subsystemName.substring(0, subsystemName.indexOf("_"));
    			    		ArrayList<String> requires = getRequirements(subsystemName);
    			    		...
    			    		for(String requirement : requires)
    			    		{
    			    			...
    			    			if(fileCondition == Condition.DOWNLOAD_COMPLETE)
    			    			{
    			    				for(int j = i+1; j < stagingFiles.length; j++)
        			    			{
    			    					String filename = stagingFiles[j].getName();
    			    			...
    			    			else if(fileCondition != Condition.UPDATE_PENDING
    			    		...
    			    		if(canAddUpdate)
    			    		{
    			    			...
    			    			{
    			    				if(stagingFiles[pos].renameTo(new File(updateDir, stagingFiles[pos].getName())))
    		                		{
    		                    		logError( getClass(), "Unable to move " + stagingFiles[pos].getName()
    		                    				+ " from " + stagingDir.getAbsolutePath() + " to "
    		                    				+ updateDir.getAbsolutePath());
    		                		}
    		                		else
    		                		{
    		                			updateFileConditions.put(stagingFiles[pos].getName(), Condition.UPDATE_PENDING);
    		                			logInfo("Moving "+ stagingFiles[pos].getName()
    		                    				+ " from " + stagingDir.getAbsolutePath() + " to "
    		                    				+ updateDir.getAbsolutePath());
    		                		}
     ...
     *
     * @param subsystem The name of the subsystem who's requirements we are checking.
     * @return A list of a required subsystems. Returns null if no connection can be made with
     * the provider.
     */
    private ArrayList<String> getRequirements(String subsystem)
    {
    	...
    	if(updater != null)
    	{
    		...
    		if (required != null)
    		{
    			...
    			{
    				ArrayList<String> temp = getRequirements(requiredSubsystem);
    				...
    				SubsystemUpdater requiredUpdater = subsystemsByName.get(requiredSubsystem);
    				if(requiredUpdater == null)
    					return null;
    				requiredSubsystem += "_" + requiredUpdater.currentVersion() + ".jar";
	...
	{
		try {
			FileUtilities.unZip(new ZipFile("C:\\Documents and Settings\\Travis Bale\\My Documents\\Downloads\\Web-CAT_1.4.0.war"), new File("C:\\Documents and Settings\\Travis Bale\\My Documents\\WEB_CAT"));
		} catch (IOException e) {
	...
	*/
     ...
     */
    public static void logError( Class<?> reference, String msg, Throwable exception )
    {
